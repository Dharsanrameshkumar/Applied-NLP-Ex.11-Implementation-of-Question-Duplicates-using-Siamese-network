...
def Siamese(text_vectorizer, vocab_size=36224, d_feature=128):
    ...
    # Embedding layer
    branch.add(tf.keras.layers.Embedding(input_dim=vocab_size, 
    ...
    # Process each input through the same branch
    branch1 = branch(input1)
    branch2 = branch(input2)
    
    # Concatenate both normalized vectors
...
def TripletLossFn(v1, v2, margin=0.25):
    ...
    # Mask to exclude positives
    mask_exclude_positives = tf.cast(
        (tf.eye(tf.shape(v1)[0]) == 1) | (negative_zero_on_duplicate <= tf.linalg.diag_part(scores)[:, None]),
        scores.dtype
    )
...
def classify(test_Q1, test_Q2, y_test, threshold, model, batch_size=64, verbose=True):
    ...
    pred = tf.concat(preds, axis=0)
    
    # Get embedding size and split the output
    _, n_feat = pred.shape
    v1 = pred[:, :n_feat//2]
    v2 = pred[:, n_feat//2:]
    ...
    # Calculate accuracy
    accuracy = tf.reduce_mean(tf.cast(y_pred == y_test_tensor, tf.float32))
    
    # Compute confusion matrix
    cm = tf.math.confusion_matrix(y_test_tensor, y_pred, num_classes=2)
